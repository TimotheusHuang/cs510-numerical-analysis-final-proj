% Compare all three methods for different initial iterate (x^(0)) using the n-by-n strictly diagonally dominant matrices
...
%     A = gallery('tridiag', n, -1,2,1);    
    ...
    for i=1:50
        x = rand(n,1);
        b = A*x;
%         [sol_jsb, num_iter_jcb, error_jcb] = jacobi(A,b,max_iter,e,0);
%         [sol_gs, num_iter_gs, error_gs] = gauss_seidel(A,b,max_iter,e,0);
%         [sol_sor, num_iter_sor, error_sor] = sor(A,b,1.024,max_iter,e,0);
%         [sol_jsb, num_iter_jcb, error_jcb] = jacobi(A,b,max_iter,e,1);
%         [sol_gs, num_iter_gs, error_gs] = gauss_seidel(A,b,max_iter,e,1);
%         [sol_sor, num_iter_sor, error_sor] = sor(A,b,1.024,max_iter,e,1);
...
title('Absolute Backward Error when Converget')
xlabel('n: Size of n-by-n Matrix A')
ylabel('Error')
legend('Jacobi','Gauss-Seidel','SOR')
hold off